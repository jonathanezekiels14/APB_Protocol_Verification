class apb_generator;
	...
	task run();
		for(int i=0;i<`num_of_transactions;i++) begin
			assert(gen_bridge_trans.randomize() == 1);
			assert(gen_slave_trans.randomize() == 1);
			gen_2_bridge_drv.put(gen_bridge_trans.copy());
			gen_2_slave_drv.put(gen_slave_trans.copy());
			$display("[GENERATOR][%0t] Transaction No: %d Randomized  ADDR = %0H | READ_WRITE = %0H | PSLVERR = %0B",$time,i+1,gen_bridge_trans.addr_in,gen_bridge_trans.write_read,gen_slave_trans.PSLVERR);
		end
	endtask
endclass
