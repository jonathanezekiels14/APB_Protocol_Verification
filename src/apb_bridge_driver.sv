		...
		for(int i=0;i<`num_of_transactions;i++) begin
			...
			fork
				begin
					...
					@(vif.bridge_drv_cb);
					vif.bridge_drv_cb.write_read <= bridge_trans.write_read;
					...
					bridge_trans.error = vif.bridge_drv_cb.error;
					vif.bridge_drv_cb.transfer <= 0;
					$display("[DRIVER - BRIDGE] [%0t] Transaction No: %d Completed Transfer",$time,i+1);
				end
			...
			disable fork;
			if(vif.PRESETn == 0) begin
				$display("[DRIVER - BRIDGE] [%0t] Transaction No: %d Reset asserted...",$time,i+1);
				@(vif.bridge_drv_cb); 
				vif.bridge_drv_cb.write_read <= 0;
				vif.bridge_drv_cb.transfer <= 0;
				vif.bridge_drv_cb.addr_in <= 0;
