class apb_scoreboard;
	...
	task run();
		for(int i=0;i<`num_of_transactions;i++) begin
			...
			if(vif.PRESETn == 0)
				$display("[SCOREBOARD] [%t] Reset Asserted Transaction No: %d",$time,i+1);
			else begin
				if(mon_trans.compare(ref_trans)) begin
					$display("[SCOREBOARD] [%t] [PASS] Transaction No: %d MATCHED",$time,i+1);
					pass_count++;
				end

				else begin
					$error("[SCOREBOARD] [%t] [FAIL] Transaction No: %d FAILED",$time,i+1);
